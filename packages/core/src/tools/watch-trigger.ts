...
export default class WatchTriggerTool implements Tool {
  ...
  async execute(
    ...
    await this.init_whisp_observer(agentContext);
    ...
    while (new Date().getTime() - start < timeout) {
      ...
      let changed = await this.has_whisp_changed(agentContext);
      if (changed == "false") {
        continue;
      }
      await this.init_whisp_observer(agentContext);
  ...
  private async init_whisp_observer(
    agentContext: AgentContext
  ): Promise<void> {
    try {
      ...
      await screenshot.call(
        ...
        () => {
          ...
          _window.has_whisp_changed = false;
          _window.whisp_observer &&
            _window.whisp_observer.disconnect();
          let whisp_observer = new MutationObserver(function (mutations) {
            _window.has_whisp_changed = true;
          });
          whisp_observer.observe(document.body, {
            childList: true,
            subtree: true,
            attributes: true,
          ...
          _window.whisp_observer = whisp_observer;
        },
        []
      );
    } catch (error) {
      console.error("Error initializing Whisp observer:", error);
    }
  }

  private async has_whisp_changed(
    agentContext: AgentContext
    ...
    try {
      ...
      let result = (await screenshot.call(
        ...
        agentContext,
        () => {
          return (window as any).has_whisp_changed + "";
        },
        []
      )) as string;
      return result as any;
    } catch (e) {
      console.error("Error checking Whisp change:", e);
      return "undefined";
    }
  }
